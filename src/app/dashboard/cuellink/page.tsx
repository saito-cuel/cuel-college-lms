    ...
    <div className="max-w-3xl mx-auto space-y-6 pb-12">
      ...
      <section className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
        ...
        <div className="divide-y divide-slate-100">
          <CourseRow date="3月4日（水）12:00"  title="所得税の新しい基礎知識" tag="全員" />
          <CourseRow date="3月7日（土）11:00"  title="ロジカルシンキング（上司への報告・説明で迷わない考え方）" tag="全員" />
          <CourseRow date="3月11日（水）20:00" title="freeeで学ぶ経理の日常業務" tag="全員" />
          <CourseRow date="3月14日（土）10:00" title="目標設定WS：ライフラインチャート" tag="全員" />
          <CourseRow date="3月14日（土）11:00" title="何から始めればいいのか：原価計算" tag="全員" />
          <CourseRow date="3月18日（水）20:00" title="キャリアの棚卸し＆強み発見WS【6名限定】" tag="全員" highlight />
          <CourseRow date="3月20日（金）21:00" title="齋藤のなんでも相談会" tag="スタンダード限定" premium noArchive />
          <CourseRow date="3月21日（土）11:00" title="実務アップデート：動画学習のその先へ" tag="全員" />
          <CourseRow date="3月23日（月）"      title="「自分という会社」のB/Sを作るWS" tag="全員" />
          <CourseRow date="3月28日（土）11:00" title="数字で未来を予測する：減損会計＆資産除去債務のキホン" tag="スタンダード限定" premium />
        </div>
      </section>
      ...
      <section className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
        ...
        <div className="divide-y divide-slate-100">
          ...
          <QandACard
            ...
            expertComment="非常に実務的で重要な論点です。現時点で「社内効率化のみ」なら通信費で問題ありません。ただし「外部販売を前提とした開発」フェーズに入った時点で、研究開発費への切り替えを検討すべきです。経営判断のタイミングと会計処理のタイミングを揃えることが実態に即した処理につながります。"
          />
        </div>
      </section>
...
function CourseRow({
  date, title, tag, premium, noArchive, highlight,
}: {
  date: string; title: string; tag: string;
  premium?: boolean; noArchive?: boolean; highlight?: boolean;
}) {
  return (
    <div className={`flex items-start gap-4 px-6 py-4 ${highlight ? 'bg-yellow-50' : ''}`}>
      ...
      <div className="flex items-center gap-1.5 flex-shrink-0">
        ...
        {premium
          ? <span className="flex items-center gap-1 text-xs bg-primary-600 text-white px-2 py-0.5 rounded-full font-medium"><Lock className="w-3 h-3" />限定</span>
          : <span className="text-xs bg-slate-100 text-slate-500 px-2 py-0.5 rounded-full">全員</span>
        }
      </div>
    </div>
  );
...
function QandACard({
  date, author, tag, question, expertComment, hearts, comments,
}: {
  date: string; author: string; tag: string; question: string;
  expertComment: string; hearts?: number; comments?: number;
}) {
  return (
    <div className="p-6 space-y-4">
      <div className="flex items-start gap-3">
        <div className="w-9 h-9 bg-slate-200 rounded-full flex items-center justify-center text-slate-500 text-xs font-bold flex-shrink-0">
          {author[0]}
        </div>
